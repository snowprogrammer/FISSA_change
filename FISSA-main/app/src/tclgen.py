...
class TCL:
    ...
    def set_nb_simu_total(self, threat_model: list, window: list):
        ...
        for threat in threat_model:
            match threat:
                ...
                case "single_bitflip_spatial_reg_multi":
                    valid_registers = [(reg1, reg2)
                                       for (reg1, size1), (reg2, size2) in combinations(zip(self.__registers_list, self.__registers_size), 2)
                                       if self.__nb_faults <= size1  and self.__nb_faults <= size2 ]
                    for fault1 in range(self.__nb_faults):
                        for fault2 in range(self.__nb_faults):
                            for index, (register_1, register_2) in enumerate(valid_registers):
                                ...
                                self.__nb_simu_total += math.comb(size_1, fault1) * math.comb(size_2, fault2) * delta_window
    ...
    def build_data_string(self):
        ...
        for win_index, window in enumerate(self.__config_data_simulator['fenetre_tir'][self.__code], start=1):
            # *** reset per-window counters BEFORE computing nb_simu_total ***
            self.__nb_simu_total = 0        # IMPORTANT: avoid cumulative counting across windows
            self.__nb_simu = 0              # reset number of generated simulations for this window
            self.__batch_number = 1         # reset batch counter for this window (if desired)

            print(f"\n--- Window {win_index}: {window} ---")
            ...
            # build unique file names for this window (avoid overwriting)
            file_str = "source\\ " + str(self.__path_file_sim) + f"{self.__code}_{self.__protection}_{self.__file_number}.tcl"
            self.build_make_list = file_str
            log_file_sim = str(self.__path_file_sim) + f"results/{self.__code}_{self.__protection}_{self.__file_number}.json"

            self.__tcl_file = self.__gen_path + f"{self.__code}_{self.__protection}_{self.__file_number}.tcl"

            # build reference and attacked sims for this window
            self.build_ref_sim(log_file_sim, window, self.__nb_simu_total, 1)
            self.build_faulted_simu(window, self.__nb_simu_total)

            # after finishing a window, increment file number so next window uses new filenames
            self.__file_number += 1
    ...
    def build_faulted_simu(self, window, nb_simulations):
        ...
        for threat in self.__threat_model:
            ...
            elif(threat == "single_bitflip_spatial_reg_multi"):
                print("\t\t >>>> Fixed bit-flip inside 2 registers")
                self.build_single_bitflip_spatial_reg_multi(window, nb_simulations)            
                ...
                for reg in self.__registers_list:
                    if(reg not in self.__config_data_simulator['avoid_register']):
                        for wreg in range(self.__registers_size[self.__registers_list.index(reg)]):
                            for start_time in range(window[0], window[1], cpu_period):
    ...
    def build_bitflip_simu(self, start_time, reg, wreg, nb_simulations):
        ...
        try:
            ...
            print("TypeError happened - single bitflip")
            exit(1)
    ...
    def build_single_bitflip_spatial(self, window, nb_simulations):
        ...
        for reg1, reg2 in combinations_list:
            for start_time in range(window[0], window[1], cpu_period):
                ...
                size_reg_0 = 0
                size_reg_1 = 0
                if("/digilent_tb/UUT/VexRiscv/RegFilePlugin_regFile" not in reg1):
                    if("/digilent_tb/UUT/VexRiscv/RegFilePlugin_regFile" not in reg2):
    ...
    def build_single_bitflip_spatial_reg_multi(self, window, nb_simulations):
        """"""
        valid_registers = [(reg1, size1, reg2, size2)
                           for (reg1, size1), (reg2, size2) in combinations(zip(self.__registers_list, self.__registers_size), 2)
                           if self.__nb_faults <= size1 and self.__nb_faults <= size2]

        nb_valid_reg = sum(1 for size in self.__registers_size if self.__nb_faults <= size)
        ...
        for index, (register_1, size_1, register_2, size_2) in enumerate(valid_registers):
            for size1 in range(int(math.pow(2,size_1))):
                bit_flipped_0=bin(size1)[2:]
                if bit_flipped_0.count('1') <= self.__nb_faults:
                    for size2 in range(int(math.pow(2,size_2))):
                        bit_flipped_1=bin(size2)[2:]
                        if bit_flipped_1.count('1') <= self.__nb_faults:
                            for start_time in range(window[0], window[1], cpu_period):
                                self.__tcl_string = list()
                                self.__nb_simu += 1
                                self.__tcl_string.append(self.__code_exec.init_sim_attacked_single_bitflip_spatial_reg_multi(self.__nb_simu, start_time, "single_bitflip_spatial_reg_multi", register_1, size_1, register_2, size_2))
                                self.__tcl_string.append(self.__inject_fault.inject_fault("single_bitflip_spatial_reg_multi", bit_flipped_0, bit_flipped_1))
                                if(self.__protection == "wop"):
                                    self.__tcl_string.append(self.__code_exec.run_sim_attacked())
                                elif(self.__protection == "hamming"):
                                    self.__tcl_string.append(self.__code_exec.run_sim_attacked_hamming())
                                self.__tcl_string.append(self.__log_data.log_sim(threat="single_bitflip_spatial_reg_multi"))
                                self.__tcl_string.append(self.__code_exec.end_sim(self.__nb_simu, nb_simulations))
                                try:
                                    self.write_tcl_file(''.join(self.__tcl_string))
                                except TypeError as e:
                                    print("TypeError happened -- single_bitflip_spatial_reg_multi\n", e)
                                    exit(1)
                                if(self.__nb_simu >= (self.__batch_max_sim * self.__batch_number)):
                                    self.__batch_number += 1
                                    self.gen_new_file(window)
    ...
    def read_register_list(self):
        ...
        try:
            ...
            print(f"File registers_{self.__protection}_{self.__implem_version}.yaml not found. Please check the installation.")
