    ...
    def inject_fault(self, threat, bit_flipped_0 = 0, bit_flipped_1 = 0, size_reg_0 = 1, size_reg_1 = 1):
        match threat:
            ...
            case "single_bitflip_spatial_reg_multi":
                return self.__single_bitflip_spatial_reg_multi(bit_flipped_0, bit_flipped_1)
...
""".format(wreg_0 = bit_flipped_0, wreg_1 = bit_flipped_1)
    ...
    def __single_bitflip_spatial_reg_multi(self, value_reg_0, value_reg_1):
        """"""
        return """
if {{$threat == "single_bitflip_spatial_reg_multi"}} {{
    set bits_attacked {value_set_reg_0}
    set value_curr_reg [examine -bin $faulted_register_0]]
    regexp {{b(.*)}} $value_curr_reg -> value
    set binary_value [binary format c* [split $value ""]]
    set binary_attack [binary format c* [split $bits_attacked ""]]
    set binary_bit_flipped_0 ""
    for {{set i 0}} {{$i < [string length $binary_value]}} {{incr i}} {{
        set bit_value [string index $binary_value $i]
        set bit_bits_attacked [string index $binary_attack $i]
        set bit_flipped [expr {{$bit_value ^ $bit_bits_attacked}}]
        append binary_bit_flipped_0 $bit_flipped
    }}
    force -freeze $faulted_register_0 $bit_flipped_0 0 -cancel "$half_periode ns"

    set bits_attacked {value_set_reg_1}
    set value_curr_reg [examine -bin $faulted_register_1]]
    regexp {{b(.*)}} $value_curr_reg -> value
    set binary_value [binary format c* [split $value ""]]
    set binary_attack [binary format c* [split $bits_attacked ""]]
    set binary_bit_flipped_1 ""
    for {{set i 0}} {{$i < [string length $binary_value]}} {{incr i}} {{
        set bit_value [string index $binary_value $i]
        set bit_bits_attacked [string index $binary_attack $i]
        set bit_flipped [expr {{$bit_value ^ $bit_bits_attacked}}]
        append binary_bit_flipped_1 $bit_flipped
    }}
